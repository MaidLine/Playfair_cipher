    ...
    public class Encrypter
    {
        ...
        public void Encryption(string word, string key)
        {
            ...
            foreach (string both in str)
            {
                ...
                if (sumbol1 == sumbol2)
                    outtext = outtext + sumbol1 + "X" + sumbol2;
        ...
        public void Descryption(string word, string key)
        {
            ...
            foreach (string both in str)
            {
                ...
                if (i_first == i_second)
                {
                    if (j_first == 0) //если символ первый в строке, раскодируем его последним символом из матрицы
                        sumbol1 = Convert.ToString(table[i_first, columns - 1]);
                    else //если символ не первый, раскодируем его стоящим слева от него
                        sumbol1 = Convert.ToString(table[i_first, j_first - 1]);
            ...
            if (wordlength % 2 == 0 && outtext[wordlength - 1] == 'Z') // если длинна слова чет, удаление Z, если оно есть 
        ...
        {
            key = key.ToUpper();
            var temp = key.Distinct(); // исключение повторяющихся букв
            key = "";
